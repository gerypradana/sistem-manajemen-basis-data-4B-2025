...
CREATE TABLE `periksa` (
  ...
  `status_transaksi` enum('aktif','pasif','non-aktif') DEFAULT NULL,
...
insert  into `periksa`(`id_periksa`,`id_pasien`,`id_dokter`,`tanggal_periksa`,`diagnosa`,`jumlah_transaksi`,`status_transaksi`) values 
(1,1,1,'2025-10-07','Flu Berat',100000.00,'aktif'),
(2,2,2,'2025-04-08','Asma',100000.00,'pasif'),
(3,3,3,'2025-04-09','Cacar Air',200000.00,'aktif'),
(4,4,4,'2025-04-10','Migrain',150000.00,'pasif'),
(5,5,5,'2025-04-10','Sakit Gigi',200000.00,'aktif'),
(6,6,6,'2025-04-11','Infeksi Mata',150000.00,'pasif'),
(7,7,7,'2025-04-11','Radang Telinga',0.00,'non-aktif'),
(8,8,8,'2025-04-12','Batuk Kronis',150000.00,'pasif'),
(9,9,9,'2025-04-12','Jantung Lemah',0.00,'non-aktif'),
(10,10,1,'2025-04-13','Demam Berdarah',100000.00,'pasif');
...
insert  into `rawat_inap`(`id_rawat_inap`,`id_pasien`,`id_ruangan`,`tanggal_masuk`,`tanggal_keluar`,`status`) values 
(1,1,1,'2025-04-06','2025-04-10 00:00:00','selesai'),
(2,2,2,'2025-04-08','2025-04-10 00:00:00','selesai'),
(3,3,3,'2025-04-09','2025-04-11 00:00:00','selesai'),
(4,4,4,'2025-04-10','2025-04-15 00:00:00','selesai'),
(5,5,5,'2025-04-11','2025-04-13 00:00:00','selesai'),
(6,6,6,'2025-04-11','2025-04-12 00:00:00','selesai'),
(7,7,7,'2025-04-12','2025-04-16 00:00:00','selesai'),
(8,8,8,'2025-04-12','2025-04-17 00:00:00','menunggu'),
(9,9,9,'2025-04-13','2025-04-14 00:00:00','menunggu'),
(10,10,2,'2025-04-14','2025-04-18 00:00:00','menunggu');
...
/* Procedure structure for procedure `edit_pasien` */

/*!50003 DROP PROCEDURE IF EXISTS  `edit_pasien` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `edit_pasien`(
    IN p_id_pasien INT,
...
BEGIN
    ...
    IF jumlah = 0 THEN
        UPDATE pasien
        SET nama_pasien = p_nama_baru,
            alamat = p_alamat_baru
        WHERE id_pasien = p_id_pasien;
    ELSE
        SELECT 'Pasien tidak dapat diubah karena masih punya rawat inap' AS pesan;
    END IF;
END */$$
DELIMITER ;
...
/* Procedure structure for procedure `mdl6_hapus_rawat_inap_lama` */

/*!50003 DROP PROCEDURE IF EXISTS  `mdl6_hapus_rawat_inap_lama` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `mdl6_hapus_rawat_inap_lama`()
BEGIN
    DELETE FROM rawat_inap
    WHERE tanggal_keluar IS NOT NULL
    AND tanggal_masuk < CURDATE() - INTERVAL 1 YEAR;
END */$$
DELIMITER ;

/* Procedure structure for procedure `Mdl6_tampil_rawat_inap_lama` */

/*!50003 DROP PROCEDURE IF EXISTS  `Mdl6_tampil_rawat_inap_lama` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `Mdl6_tampil_rawat_inap_lama`(IN pilihan VARCHAR(10))
BEGIN
    IF pilihan = '3 bulan' THEN
        SELECT * FROM rawat_inap
        WHERE tanggal_masuk >= CURDATE() - INTERVAL 3 month;
    ELSE
        SELECT 'Pilihan tidak valid' AS pesan;
    END IF;
END */$$
DELIMITER ;

/* Procedure structure for procedure `tampilkan_transaksi_berhasil` */

/*!50003 DROP PROCEDURE IF EXISTS  `tampilkan_transaksi_berhasil` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `tampilkan_transaksi_berhasil`()
BEGIN
    DECLARE done INT DEFAULT FALSE;
    DECLARE v_id INT;
    DECLARE v_tanggal DATE;
    DECLARE v_diagnosa VARCHAR(100);
    DECLARE v_jumlah DECIMAL(10,2);

    -- Cursor untuk transaksi berhasil dalam 1 bulan terakhir
    DECLARE cur CURSOR FOR
        SELECT id_periksa, tanggal_periksa, diagnosa, jumlah_transaksi
        FROM periksa
        WHERE jumlah_transaksi > 0
        AND tanggal_periksa >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH);
    ...
    transaksi_loop: LOOP
        FETCH cur INTO v_id, v_tanggal, v_diagnosa, v_jumlah;
        IF done THEN
            LEAVE transaksi_loop;
        END IF;

        -- Tampilkan hasil baris per baris
        SELECT 
            CONCAT('ID: ', v_id, ', Tanggal: ', v_tanggal, ', Diagnosa: ', v_diagnosa, ', Jumlah: ', v_jumlah) AS detail_transaksi;
...
/* Procedure structure for procedure `ubah_status_7_rawat` */

/*!50003 DROP PROCEDURE IF EXISTS  `ubah_status_7_rawat` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `ubah_status_7_rawat`()
BEGIN
    UPDATE rawat_inap
    SET status = 'selesai'
...
/* Procedure structure for procedure `update_status_transaksi_periksa` */

/*!50003 DROP PROCEDURE IF EXISTS  `update_status_transaksi_periksa` */;

DELIMITER $$

/*!50003 CREATE DEFINER=`root`@`localhost` PROCEDURE `update_status_transaksi_periksa`()
BEGIN
    DECLARE id_min INT;
    DECLARE id_max INT;
    DECLARE id_mid INT;
    
    -- Ambil id transaksi terkecil dan terbesar dalam 1 bulan terakhir
    SELECT id_periksa INTO id_min
    FROM periksa
    WHERE tanggal_periksa >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)
    ORDER BY jumlah_transaksi ASC
    LIMIT 1;

    SELECT id_periksa INTO id_max
    FROM periksa
    WHERE tanggal_periksa >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)
    ORDER BY jumlah_transaksi DESC
    LIMIT 1;
    
    -- Ambil transaksi tengah (jumlah sedang)
    SELECT id_periksa INTO id_mid
    FROM periksa
    WHERE tanggal_periksa >= DATE_SUB(CURDATE(), INTERVAL 1 MONTH)
    AND id_periksa NOT IN (id_min, id_max)
    ORDER BY jumlah_transaksi
    LIMIT 1;
    
    -- Update status masing-masing
    UPDATE periksa SET status_transaksi = 'non-aktif' WHERE id_periksa = id_min;
    UPDATE periksa SET status_transaksi = 'pasif' WHERE id_periksa = id_mid;
    UPDATE periksa SET status_transaksi = 'aktif' WHERE id_periksa = id_max;
END */$$
DELIMITER ;
