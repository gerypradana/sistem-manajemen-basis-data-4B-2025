...
(3,'seni kerajinan tangan','Usaha yang bergerak di bidang pembuatan produk kerajinan tangan'),
...
CREATE TABLE `produk_umkm` (
  ...
  KEY `fk_produk_umkm` (`id_umkm`),
  CONSTRAINT `fk_produk_umkm` FOREIGN KEY (`id_umkm`) REFERENCES `umkm` (`id_umkm`)
...
(37,1,'Kopi Arabika',NULL,25000.00);
...
CREATE TABLE `umkm` (
  ...
  PRIMARY KEY (`id_umkm`),
  KEY `fk_umkm_pemilik` (`id_pemilik`),
  KEY `fk_umkm_kategori` (`id_kategori`),
  KEY `fk_umkm_skala` (`id_skala`),
  KEY `fk_umkm_kabupaten` (`id_kabupaten_kota`),
  CONSTRAINT `fk_umkm_kabupaten` FOREIGN KEY (`id_kabupaten_kota`) REFERENCES `kabupaten_kota` (`id_kabupaten_kota`),
  CONSTRAINT `fk_umkm_kategori` FOREIGN KEY (`id_kategori`) REFERENCES `kategori_umkm` (`id_kategori`),
  CONSTRAINT `fk_umkm_pemilik` FOREIGN KEY (`id_pemilik`) REFERENCES `pemilik_umkm` (`id_pemilik`),
  CONSTRAINT `fk_umkm_skala` FOREIGN KEY (`id_skala`) REFERENCES `skala_umkm` (`id_skala`)
...
(16,'Toko Serba Ada',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,5,NULL,NULL,NULL,'2025-04-25');
...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_umkm_per_kota` AS select `kk`.`nama_kabupaten_kota` AS `nama_kabupaten_kota`,count(`u`.`id_umkm`) AS `jumlah_umkm` from (`umkm` `u` join `kabupaten_kota` `kk` on(`u`.`id_kabupaten_kota` = `kk`.`id_kabupaten_kota`)) group by `kk`.`nama_kabupaten_kota` */;
